...
function render() {
  ...
  const views = { race: raceView, add: addView, review: reviewView, entries: () => `<section class="card table-card"><div class="table-title"><h2>The group’s spending log</h2></div>${entriesView()}</section>`, slack: slackView, rules: rulesView, account: accountView };
  app.innerHTML = `${header()}<main class="shell"><div class="intro row between"><div><div class="eyebrow muted">October 1–31, 2026 · ${esc(data.challenge.currency)}</div><h1 style="margin-top:10px">The race to spend less.</h1><p class="muted">Every dollar counts. Keep your line low.</p></div><button class="button" data-tab="add">＋ Add spending</button></div><nav class="tabs" aria-label="Challenge">${[['race', 'The race'], ['entries', 'Spending log'], ['slack', 'Slack commands'], ['rules', 'Ground rules'], ['account', 'Account']].map(([id, title]) => `<button class="tab" data-tab="${id}" ${tab === id ? 'aria-current="page"' : ''}>${title}</button>`).join('')}</nav><div id="view">${(views[tab] || raceView)()}</div><footer class="footer row between wrap"><span>October Club · A little less, together.</span><button class="button ghost small" data-action="refresh">Refresh totals</button></footer></main>`;
}
